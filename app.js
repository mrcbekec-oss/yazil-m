...
document.querySelector('h1').addEventListener('click', () => {
    ...
    const examples = {
        ...
        weather: {
            html: '<div class="weather-card">\n  <div class="city">İstanbul</div>\n  <div class="temp">24°C</div>\n  <div class="desc">Güneşli</div>\n  <div class="details">\n    <div>Nem: %40</div>\n    <div>Rüzgar: 12 km/s</div>\n  </div>\n</div>',
            css: 'body { background: linear-gradient(135deg, #1e3a8a, #3b82f6); display: flex; justify-content: center; align-items: center; min-height: 100vh; font-family: sans-serif; }\n.weather-card { background: rgba(255,255,255,0.1); backdrop-filter: blur(20px); border: 1px solid rgba(255,255,255,0.2); padding: 40px; border-radius: 30px; color: white; text-align: center; box-shadow: 0 20px 50px rgba(0,0,0,0.2); }\n.city { font-size: 24px; opacity: 0.8; }\n.temp { font-size: 80px; font-weight: bold; margin: 10px 0; }\n.desc { font-size: 20px; margin-bottom: 20px; }\n.details { display: flex; gap: 20px; font-size: 14px; opacity: 0.7; }',
            js: '// Statik önizleme örneği'
        },
        draw: {
            html: '<canvas id="paint" width="400" height="400"></canvas>\n<div class="controls">\n  <button onclick="clearCanvas()">Temizle</button>\n</div>',
            css: 'body { background: #f0f2f5; display: flex; flex-direction: column; justify-content: center; align-items: center; min-height: 100vh; }\ncanvas { background: white; border-radius: 12px; box-shadow: 0 5px 15px rgba(0,0,0,0.1); cursor: crosshair; }\n.controls { margin-top: 20px; }',
            js: 'const canvas = document.getElementById("paint");\nconst ctx = canvas.getContext("2d");\nlet drawing = false;\ncanvas.addEventListener("mousedown", () => drawing = true);\ncanvas.addEventListener("mouseup", () => drawing = false);\ncanvas.addEventListener("mousemove", (e) => {\n  if (!drawing) return;\n  ctx.lineWidth = 5;\n  ctx.lineCap = "round";\n  ctx.strokeStyle = "#6366f1";\n  ctx.lineTo(e.clientX - canvas.offsetLeft, e.clientY - canvas.offsetTop);\n  ctx.stroke();\n  ctx.beginPath();\n  ctx.moveTo(e.clientX - canvas.offsetLeft, e.clientY - canvas.offsetTop);\n});\nfunction clearCanvas() { ctx.clearRect(0, 0, canvas.width, canvas.height); ctx.beginPath(); }'
        },
        quiz: {
            html: '<div class="quiz-box">\n  <h2 id="question">Gökyüzü ne renktir?</h2>\n  <div class="options">\n    <button onclick="check(false)">Yeşil</button>\n    <button onclick="check(true)">Mavi</button>\n    <button onclick="check(false)">Kırmızı</button>\n  </div>\n  <p id="result"></p>\n</div>',
            css: 'body { background: #6366f1; display: flex; justify-content: center; align-items: center; min-height: 100vh; font-family: sans-serif; }\n.quiz-box { background: white; padding: 30px; border-radius: 20px; width: 300px; text-align: center; }\n.options { display: flex; flex-direction: column; gap: 10px; margin-top: 20px; }\nbutton { padding: 12px; border: 1px solid #ddd; border-radius: 8px; background: white; cursor: pointer; transition: 0.3s; }\nbutton:hover { background: #f0f2f5; }',
            js: 'function check(isCorrect) {\n  const res = document.getElementById("result");\n  res.textContent = isCorrect ? "Doğru! 🎉" : "Yanlış! ❌";\n  res.style.color = isCorrect ? "green" : "red";\n}'
        },
        login: {
            html: '<div class="login-card">\n  <h2>Hoş Geldiniz</h2>\n  <input type="text" placeholder="Kullanıcı Adı">\n  <input type="password" placeholder="Şifre">\n  <button>Giriş Yap</button>\n  <a href="#">Şifremi Unuttum</a>\n</div>',
            css: 'body { background: linear-gradient(135deg, #0f172a, #1e293b); display: flex; justify-content: center; align-items: center; min-height: 100vh; font-family: sans-serif; }\n.login-card { background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.1); padding: 40px; border-radius: 24px; width: 320px; text-align: center; backdrop-filter: blur(10px); }\nh2 { color: white; margin-bottom: 30px; }\ninput { width: 100%; padding: 12px; margin-bottom: 15px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); background: rgba(0,0,0,0.2); color: white; }\nbutton { width: 100%; padding: 12px; border-radius: 8px; border: none; background: #6366f1; color: white; font-weight: bold; cursor: pointer; }\na { display: block; margin-top: 15px; color: #94a3b8; font-size: 12px; text-decoration: none; }',
            js: 'document.querySelector("button").addEventListener("click", () => alert("Giriş denemesi!"));'
        }
    };
